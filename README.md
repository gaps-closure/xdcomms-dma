...
The socket pointer is not needed by XDCOMMS-lib (only used by HAL), so may be set to NULL.
...
- Shared Memory (SHM) mapped regions of the host computer: as a stand-in for the shared memory on the ESCAPE FPGA board. 
