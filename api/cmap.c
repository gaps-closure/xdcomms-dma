...
void cmap_print(codec_map *cmap_root) {
  codec_map  *cm;
  fprintf(stderr, "%s: ", __func__);
  for(cm = cmap_root; cm->valid != 0; cm++) cmap_print_one(cm);
  fprintf(stderr, "\n");
}

codec_map *cmap_find(int data_type, codec_map *cmap_root) {
  codec_map  *cm;
  for(cm = cmap_root; cm->valid != 0; cm++) {
...
void cmap_encode(uint8_t *data, uint8_t *buff_in, size_t *buff_len, gaps_tag *tag, codec_map *cmap_root) {
  codec_map  *cm = cmap_find(tag->typ, cmap_root);
