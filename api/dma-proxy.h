 ...
 * under the License.
 */
 /* This header file is shared by the DMA Proxy test application and the GE
 * DMA Proxy device driver. It defines the shared interface to allow DMA
 * transfers to be done from user space.
 *
...
#define BUFFER_SIZE (128 * 1024)	 	    /* Channel buffer bytes - must match driver exactly 2^17 = 128KB = 0x20000 */
#ifdef SUE_DONIMOUS
...
#define PKT_G1_ADU_SIZE_MAX  65528     // Max packet bytes with 16-bit BW data_len = 2^16 - 8 (see bw header)
...
/* BW packet format (MIND format) - 2^16 = 64KB = 0x10000 (must fit in channel buffer) */
