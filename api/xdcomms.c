...
void get_dev_val(unsigned long *val, char *env_val, unsigned long def_val_dma, unsigned long def_val_shm, char *dev_type) {
  ...
  log_trace("%s: env_val = %s %x def = [d=%d s=%d]", __func__, env_val, *valm def_val_dma, def_val_shm);
}
...
void *rcvr_thread_function(thread_args *vargs) {
  ...
  log_debug("THREAD %s starting: fd=%d base_id=%d cb_ptr=%p", __func__, cp->fd, vargs->buffer_id_start, dma_cb_ptr);
