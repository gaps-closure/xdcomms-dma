...
void chan_print(chan *cp) {
  ...
  fprintf(stderr, "  ret=%d every %d ns newd=%d rx_buf_ptr=%p len=%lx tid=%d tag=<%d,%d,%d>", cp->retries, RX_POLL_INTERVAL_NSEC, cp->pinfo.newd, cp->pinfo.data, cp->pinfo.data_len, cp->pinfo.tid, cp->pinfo.tag->mux, cp->pinfo.tag->sec, cp->pinfo.tag->typ);
}
