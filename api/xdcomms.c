...
void chan_print(chan *cp) {
  ...
  log_trace("                  mmap len=0x%x [paddr=0x%08x vaddr=0x%08x offset=0x%x protect=0x%x flags=0x%x]",  cp->mm.phys_addr, cp->mm.virt_addr, cp->mm.len, cp->mm.offset, cp->mm.prot, cp->mm.flags);
