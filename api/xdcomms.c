...
void chan_print(chan *cp) {
  ...
  fprintf(stderr, "  mmap len=0x%x [pa=0x%lx va=%p off=0x%lx prot=0x%x flag=0x%x]",  cp->mm.len, cp->mm.phys_addr, cp->mm.virt_addr, cp->mm.offset, cp->mm.prot, cp->mm.flags);
